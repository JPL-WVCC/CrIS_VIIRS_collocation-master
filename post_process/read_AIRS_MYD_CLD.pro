...
cld_list1=['Cloud_Top_Temperature_1km','Cloud_Top_Pressure_1km','Cloud_Optical_Thickness','Cloud_Effective_Radius','Cloud_Water_Path']
cld_list2=['Cloud_Mask','Cloud_Phase_Cloud_Top_Properties','Cloud_Phase_Optical_Properties']

MODISL2dir='/raid15/qyue/VIIRS/MODIS/201601/MYD06/'
...
spawn,'ls '+MODISL2dir+string(iday,format='(i3.3)')+'/MYD06_L2.A'+mode1+'*.sav',MODIScldfiles
...
for ifile=0,23 do begin
   ...
   for iVgran=0,ngran-1 do begin 
         ...
         ;     content_list=[cld_list1,cld_list2])
          restore,modisfile
          if okfile eq 0 then begin
             ctp=cloud_top_pressure_1km.value*cloud_top_pressure_1km.scale_factor.data[0]+cloud_top_pressure_1km.add_offset.data[0]
             cod=cloud_optical_thickness.value*cloud_optical_thickness.scale_factor.data[0]+cloud_optical_thickness.add_offset.data[0]
             Re=cloud_effective_radius.value*cloud_effective_radius.scale_factor.data[0]+cloud_effective_radius.add_offset.data[0]
             ctt=cloud_top_temperature_1km.value*cloud_top_temperature_1km.scale_factor.data[0]+cloud_top_temperature_1km.add_offset.data[0]
             CWP=cloud_water_path.value*cloud_water_path.scale_factor.data[0]+cloud_water_path.add_offset.data[0]
             ;cphasetop=data.Cloud_Phase_Cloud_Top_Properties
             cphaseopt=Cloud_Phase_Optical_Properties.value
             cmaskt=cloud_mask_1km.value
             data=0        
          endif
          if okfile GE 1 then begin  
             ctp=concat(ctp,cloud_top_pressure_1km.value*cloud_top_pressure_1km.scale_factor.data[0]+cloud_top_pressure_1km.add_offset.data[0],dimension=1)
             cod=concat(cod,cloud_optical_thickness.value*cloud_optical_thickness.scale_factor.data[0]+cloud_optical_thickness.add_offset.data[0],dimension=1)
             Re=concat(Re,cloud_effective_radius.value*cloud_effective_radius.scale_factor.data[0]+cloud_effective_radius.add_offset.data[0],dimension=1)
             ctt=concat(ctt,cloud_top_temperature_1km.value*cloud_top_temperature_1km.scale_factor.data[0]+cloud_top_temperature_1km.add_offset.data[0],dimension=1)
             CWP=concat(cwp,cloud_water_path.value*cloud_water_path.scale_factor.data[0]+cloud_water_path.add_offset.data[0],dimension=1)
             ;cphasetop=data.Cloud_Phase_Cloud_Top_Properties
             cphaseopt=concat(cphaseopt,Cloud_Phase_Optical_Properties.value,dimension=1)
             cmaskt=concat(cmaskt,cloud_mask_1km.value,dimension=2)
             data=0 
          endif
   endfor
   ...
   MYD06_ISCCP_Hist1=fltarr(nbin_tau,nbin_ctp,result[0],result[1])*!values.f_nan 
   MYD06_CLD_Mask1=fltarr(5,result[0],result[1])*!values.f_nan
   ;MYD06_CLDTOP_Phase1=fltarr(5,result[0],result[1])*!values.f_nan
   MYD06_OPT_Phase1=fltarr(5,result[0],result[1])*!values.f_nan
   MYD06_CTP_moment1=fltarr(4,result[0],result[1])*!values.f_nan
   MYD06_COD_moment1=fltarr(4,result[0],result[1])*!values.f_nan
   MYD06_CTT_moment1=fltarr(4,result[0],result[1])*!values.f_nan
   MYD06_CWP_moment1=fltarr(4,result[0],result[1])*!values.f_nan
   MYD06_Re_moment1=fltarr(4,result[0],result[1])*!values.f_nan   
   MYD06_Re_ISCCPHist1=fltarr(nbin_tau,nbin_ctp,result[0],result[1])*!values.f_nan
   MYD06_SFC_Mask1=fltarr(10,result[0],result[1])*!values.f_nan
 ...
 ;  frame = findcol / (result[1]*result[0])
   for i=0,nfindcol-1 do begin
       ...
       for im=0,4 do begin
           ...
           MYD06_CLD_Mask1[im,col[i],row[i]]=nfind
           find=where(cphaseopt[dx,dy] eq im,nfind)
           MYD06_OPT_Phase1[im,col[i],row[i]]=nfind
...
;set up sfc mask:
       for im=0,1 do begin
           ...
           MYD06_SFC_Mask1[im,col[i],row[i]]=nfind
       endfor
       for im=0,1 do begin
           find=where(glintmask[dx,dy] eq im,nfind)
           MYD06_SFC_Mask1[im+2,col[i],row[i]]=nfind
       endfor
       for im=0,1 do begin
           find=where(icemask[dx,dy] eq im,nfind)
           MYD06_SFC_Mask1[im+4,col[i],row[i]]=nfind
       endfor
       for im=0,3 do begin
           find=where(sfcmask[dx,dy] eq im,nfind)
           MYD06_SFC_Mask1[im+6,col[i],row[i]]=nfind
       endfor
       ...
       for itau=0,nbin_tau-1 do begin
           for ictp=0,nbin_ctp-1 do begin
               ...
               MYD06_ISCCP_Hist1[itau,ictp,col[i],row[i]]=nfind
               ...
               if nfind GE 1 then MYD06_Re_ISCCPHist1[itau,ictp,col[i],row[i]]=mean(re1[find],/nan)
           endfor
       endfor
       MYD06_CTP_moment1[*,col[i],row[i]]=moment(ctp[dx,dy],/nan)
       MYD06_COD_moment1[*,col[i],row[i]]=moment(COD[dx,dy],/nan)
       MYD06_CTT_moment1[*,col[i],row[i]]=moment(CTT[dx,dy],/nan)
       MYD06_Re_moment1[*,col[i],row[i]]=moment(Re[dx,dy],/nan)
       MYD06_CWP_moment1[*,col[i],row[i]]=moment(CWP[dx,dy],/nan)
   ...
   for igran=0,9 do begin
      ...
      MYD06_ISCCP_Hist=MYD06_ISCCP_Hist1[*,*,*,igran*135:(igran+1)*135-1]
      MYD06_CLD_Mask=MYD06_CLD_Mask1[*,*,igran*135:(igran+1)*135-1]
    ;  MYD06_CLDTOP_Phase=MYD06_CLDTOP_Phase1[*,*,igran*135:(igran+1)*135-1]
      MYD06_OPT_Phase=MYD06_OPT_Phase1[*,*,igran*135:(igran+1)*135-1]
      MYD06_CTP_moment=MYD06_CTP_moment1[*,*,igran*135:(igran+1)*135-1]
      MYD06_COD_moment=MYD06_COD_moment1[*,*,igran*135:(igran+1)*135-1]
      MYD06_CTT_moment=MYD06_CTT_moment1[*,*,igran*135:(igran+1)*135-1]
      MYD06_CWP_moment=MYD06_CWP_moment1[*,*,igran*135:(igran+1)*135-1]
      MYD06_Re_moment=MYD06_Re_moment1[*,*,igran*135:(igran+1)*135-1]
      MYD06_Re_ISCCPHist=MYD06_Re_ISCCPHist1[*,*,*,igran*135:(igran+1)*135-1]
      MYD06_SFC_Mask=MYD06_SFC_MAsk1[*,*,igran*135:(igran+1)*135-1]
      save,AIRS_lon,AIRS_lat,AIRS_tai93,MYD06_SFC_Mask,MYD06_ISCCP_Hist,MYD06_Re_ISCCPHist,$
        MYD06_CLD_Mask,MYD06_CTP_moment,MYD06_COD_moment,MYD06_OPT_Phase,MYD06_CTT_moment,MYD06_Re_moment,MYD06_CWP_moment,$
        /compress,filename=outdir+'MYD06_Cloud_onAIRS_'+mode+'_G'+string(ifile*10+igran+1,format='(i3.3)')+'.sav'
  endfor
