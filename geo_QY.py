...
##################################################################################### 
def match_cris_viirs_QY(crisLos, crisPos, viirsPos, cristime,viirstime):
    """
    ...
    cristimebig=np.repeat(cristime[:,:,np.newaxis],9,axis=2)
...
def read_viirs_geo (filelist, ephemeris=False, hgt=False):
    ...
    with open(filelist[0], 'rU',encoding="latin-1") as fs:
...
def read_viirs_sdr (filelist):
    ...
    with open(filelist[0], 'rU',encoding="latin-1") as fn:
...
## read NASA CrIS files     
def read_nasa_cris_geo (filelist, ephemeris = False, rad = True):
    
    """
    Read JPSS CrIS Geo files and return Longitude, Latitude, SatelliteAzimuthAngle, SatelliteRange, SatelliteZenithAngle.
        if ephemeris=True, then return forTime, midTime, satellite position, velocity, attitude 
    """
    geos = [nc4.Dataset(filename) for filename in filelist]
    
    
    if ephemeris == False:  
        Latitude  = np.concatenate([f.variables['lat'][:] for f in geos])
        Longitude = np.concatenate([f.variables['lon'][:] for f in geos])
        SatelliteAzimuthAngle = np.concatenate([f.variables['sat_azi'][:] for f in geos])
        SatelliteRange = np.concatenate([f.variables['sat_range'][:] for f in geos])
        SatelliteZenithAngle = np.concatenate([f.variables['sat_zen'][:] for f in geos])
        real_lw = np.concatenate([f.variables['rad_lw'][:] for f in geos]) 
        tai93 = np.concatenate([f.variables['obs_time_tai93'][:] for f in geos])
        [f.close() for f in geos]   
        if rad: 
            return Longitude, Latitude, SatelliteAzimuthAngle, SatelliteRange, SatelliteZenithAngle, tai93, real_lw
        else:
            return Longitude, Latitude, SatelliteAzimuthAngle, SatelliteRange, SatelliteZenithAngle, tai93
...
## READ NASA VIIRS Geo files
 
def read_nasa_viirs_geo (filelist, ephemeris=False, hgt=True):
   ...
   # geos = [h5py.File(filename, 'r') for filename in filelist]
    ...
    if not ephemeris:
        ...
        SatelliteZenithAngle = np.concatenate([f.groups['geolocation_data'].variables['sensor_zenith'][:] for f in geos])
        Height = np.concatenate([f.groups['geolocation_data'].variables['height'][:] for f in geos])
        sttime= np.concatenate([np.full(
                f.groups['geolocation_data'].variables['height'][:].shape,np.mean(
                f.groups['scan_line_attributes'].variables['ev_mid_time'][:])) for f in geos])
       # pdb.set_trace()
        [f.close() for f in geos]
        if hgt: 
            return Longitude, Latitude, SatelliteAzimuthAngle, SatelliteRange, SatelliteZenithAngle, Height,sttime
        else: 
            return Longitude, Latitude, SatelliteAzimuthAngle, SatelliteRange, SatelliteZenithAngle       
    if ephemeris: 
        MidTime  = np.concatenate([f['All_Data'][CollectionName]['MidTime'] [:] for f in geos])
...
# output:
# index_x: scan line number
# index_y: foot print number
#
# input:
# mx:
# my:
#
def find_match_index_QY (cris_los, cris_sat, viirs_pos_in, cris_time,viirs_time, \
                      mx, my, fovDia=0.963):
        ...
        for i in range(0, mx.size):
                ...
                viirs_pos = viirs_pos_in[yb:ye, xb:xe, : ]
                viirs_Qa  = abs(viirs_time[yb:ye, xb:xe]-cris_time[0,i])
                viirs_los = viirs_pos  - cris_sat[i, :]
                temp = np.dot(viirs_los, cris_los[i, :])
                temp = temp / LA.norm(viirs_los, axis=2)
                cos_angle = temp / LA.norm(cris_los[i, :])
#add time requirement less than 30min difference
#                iy, ix = np.where ( (viirs_Qa <= 720.) & (cos_angle > cos_half_fov) )
                iy, ix = np.where (  (cos_angle > cos_half_fov) )
#                iiy=np.unique(iy)
...
'''    
 max(ix[np.where(iy[:] == np.unique(iy)[ff])] for ff in range(np.unique(iy).size))       

print( max(ix[np.where(iy[:] == np.unique(iy)[ff]) for ff in range(np.unique(iy).size))] )
'''    
