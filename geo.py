...
def read_cris_geo (filelist, ephemeris = False):
    ...
    if ephemeris == False:  # ONLY replace this by netcdf4 reading
...
####################################################################################    
## read CrIS GOE from NASA CrIS netcdf4 files 
def nc_read_cris_geo (filelist):

    """
    Read JPSS CrIS Geo files and return Longitude, Latitude, SatelliteAzimuthAngle, SatelliteRange, SatelliteZenithAngle.
    """

    geos = [Dataset(filename) for filename in filelist]

    ### Latitude  = np.concatenate([f['All_Data']['CrIS-SDR-GEO_All']['Latitude'] [:] for f in geos])
    Latitude  = np.concatenate([dataset.variables['lat'][:].data for dataset in geos])

    ### Longitude = np.concatenate([f['All_Data']['CrIS-SDR-GEO_All']['Longitude'][:] for f in geos])
    Longitude  = np.concatenate([dataset.variables['lon'][:].data for dataset in geos])

    ### SatelliteAzimuthAngle = np.concatenate([f['All_Data']['CrIS-SDR-GEO_All']['SatelliteAzimuthAngle'][:] for f in geos])
    SatelliteAzimuthAngle  = np.concatenate([dataset.variables['sat_azi'][:].data for dataset in geos])

    ### SatelliteRange = np.concatenate([f['All_Data']['CrIS-SDR-GEO_All']['SatelliteRange'][:] for f in geos])
    SatelliteRange  = np.concatenate([dataset.variables['sat_range'][:].data for dataset in geos])

    ### SatelliteZenithAngle = np.concatenate([f['All_Data']['CrIS-SDR-GEO_All']['SatelliteZenithAngle'][:] for f in geos])
    SatelliteZenithAngle  = np.concatenate([dataset.variables['sat_zen'][:].data for dataset in geos])
...
def read_viirs_geo (filelist, ephemeris=False, hgt=False):
    ...
    if not ephemeris: # replace this by netcdf4 reading
...
#################################################################
## READ VIIRS Geo from NASA VIIRS netcdf4 files
def nc_read_viirs_geo (filelist):
    ...
    ### Latitude  = np.concatenate([f['All_Data'][CollectionName]['Latitude'][:]  for f in geos])
    Latitude  = np.concatenate([dataset.groups['geolocation_data'].variables['latitude'][:].data for dataset in geos])

    ### Longitude = np.concatenate([f['All_Data'][CollectionName]['Longitude'][:] for f in geos])
    Longitude  = np.concatenate([dataset.groups['geolocation_data'].variables['longitude'][:].data for dataset in geos])

    ### SatelliteAzimuthAngle = np.concatenate([f['All_Data'][CollectionName]['SatelliteAzimuthAngle'][:] for f in geos])
    SatelliteAzimuthAngle  = np.concatenate([dataset.groups['geolocation_data'].variables['sensor_azimuth'][:].data for dataset in geos])

    ### SatelliteRange = np.concatenate([f['All_Data'][CollectionName]['SatelliteRange'][:] for f in geos])
    SatelliteRange  = np.concatenate([dataset.groups['geolocation_data'].variables['range'][:].data for dataset in geos])

    ### SatelliteZenithAngle = np.concatenate([f['All_Data'][CollectionName]['SatelliteZenithAngle'][:] for f in geos])
    SatelliteZenithAngle  = np.concatenate([dataset.groups['geolocation_data'].variables['sensor_zenith'][:].data for dataset in geos])
...
if __name__ == "__main__":
  ...
  if True:
    filelist = ['/peate_archive/.data6/Ops/snpp/gdisc/2/2015/06/01/crisl1b/SNDR.SNPP.CRIS.20150601T2130.m06.g216.L1B_NSR.std.v02_05.G.180904200851.nc',
                '/peate_archive/.data6/Ops/snpp/gdisc/2/2015/06/01/crisl1b/SNDR.SNPP.CRIS.20150601T2248.m06.g229.L1B_NSR.std.v02_05.G.180904202733.nc']
    r1 = nc_read_cris_geo(filelist)
    print ('r1: ', r1)

  if True:
  ### if False:
    filelist = ['/raid15/qyue/VIIRS/VIIRS/20150601/VNP03MOD.A2015152.2318.001.2017261064549.nc',
                '/raid15/qyue/VIIRS/VIIRS/20150601/VNP03MOD.A2015152.2142.001.2017261064453.nc']
    r1 = nc_read_viirs_geo(filelist)
    print ('r1: ', r1)
